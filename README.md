...
Provisioned via Terraform (`terraform/`, using the `snowflakedb/snowflake` provider). Requires admin-level Snowflake credentials (`ACCOUNTADMIN`), supplied as `SNOWFLAKE_*` environment variables — see `terraform/providers.tf`.
